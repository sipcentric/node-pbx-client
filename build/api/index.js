...
class Nimvelo {
  ...
  static _getAuthHeader(username, password) {
    ...
    const encodedCredentials = js_base64_1.Base64.encode(
      `${username}:${password}`,
    );
  ...
  getHeaders() {
    return Object.assign({}, this.options.requestOptions.headers, {
  ...
  _request(method, url, params = {}, callback) {
    ...
    const fetchRequestOptions = Object.assign({}, this.options.requestOptions, {
      method,
  ...
  _formatGetResponse(response, parent) {
    if (!guards_1.isApiItem(response)) {
      ...
      const { nextPage, prevPage, items } = response,
        temp = __rest(response, ['nextPage', 'prevPage', 'items']);
      const meta = temp;
