...
const instantiate = (passedConfig, modules) => {
    const { JsSIP, // Main JsSIP module
    DigestAuthentication, RequestSender, SIPMessage, Transactions, } = modules;
    class WebRTC extends JsSIP.UA {
        constructor(options) {
            ...
            this.receiveRequest = (request) => {
                ...
                if (request.method === JsSIP.C.NOTIFY &&
                    ...
                    new Transactions.NonInviteServerTransaction(this, this.transport, request);
                    try {
        ...
        emitUserStateChanged(user, state) {
            ...
            const readableState = this.dialogStateMap.get(normalizedState) || 'UNKNOWN';
        ...
        updateSubscriptionState(request) {
            if (!request.headers['Subscription-State'])
                return;
            const subscriptionStateHeader = request.headers['Subscription-State'][0].raw;
        ...
        subscribeToUser(user, replacesSubscription, extraHeaders) {
        ...
        sendSubscribeRequest(ruri, options, headers) {
            const extraHeaders = [
            ...
            const request = new SIPMessage.OutgoingRequest(JsSIP.C.SUBSCRIBE, ruri, this, options, extraHeaders);
            const eventHandlers = {
                request,
        ...
        subscribeResponseHandler(response) {
             ...
             */
            if ([401, 407].includes(statusCode) &&
