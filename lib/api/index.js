...
class Sipcentric {
  ...
  async getUA(config) {
    ...
    if (!webRTCConfig.username || !webRTCConfig.password) {
      ...
      if (config.customerId) {
        const chosenCustomer = customers.items.find(x => x.id === `${config.customerId}`);
      ...
      if (!extensionId) {
        ...
        if (!linkedUser) {
          throw new Error(`No linkedUser found for this user on this customer (${customer.id})`);
        }
        ...
        if (!defaultExtension) {
          throw new Error(`No default extension set on linkedUser (${linkedUser.id}) on customer (${customer.id})`);
        }
    ...
    return new WebRTC(webRTCConfig);
  }
  ...
  _request(method, url, params = {}, callback) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.request(options, (err, response, data) => {
        if (err) {
        ...
        if (parsedData && typeof parsedData.errors !== 'undefined') {
          ...
          const error = new Error('Errors in response body');
          error.statusCode = response.statusCode;
          error.responseBody = parsedData;
          error.response = response;

          reject(error);
        } else if (parsedData && typeof parsedData.validationErrors !== 'undefined') {
          // If there are some validation errors returned, reject

          const error = new Error('Validation errors in response body');
          error.statusCode = response.statusCode;
          error.responseBody = parsedData;
          error.response = response;
          ...
          const error = new Error('Response has non-success status code');
          error.statusCode = response.statusCode;
          error.responseBody = parsedData;
          error.response = response;
        } else {
  ...
  _deleteRepresentation(object, callback) {
    ...
    return new Promise((resolve, reject) => {
      this._request('delete', url, object).then((data) => {
        const resolveData = {
          ...data.parsedData,
          _response: data.response,
        };
