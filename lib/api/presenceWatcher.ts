...
class PresenceWatcher {
  ...
  _getDialogState(notification: any) {
    // Parse our xml body
    const xml = notification.request.body;
    const json: any = parser.toJson(xml, {
      object: true,
    });
  ...
  async _getExtensionCredentials(extensionId: any) {
    ...
    const { username, password } = await phone.sip.get();
  ...
  async subscribe(options: any = {}) {
    ...
    const subscriptions = targets.map((target: string) => {
      // Make sure we've got a 6 digit username
      const paddedTarget = target.padStart(6, '0');

      // Subscribe
      const subscription = ua.subscribe(
        `${paddedTarget}@${domain}`,
        'dialog',
        {},
      );
      ...
      subscription.on('notify', (notification: any) => {
        // Get a more readable state
