...
class Nimvelo {
  ...
  _buildUrl(base, type, customerId, resourceId) {
    ...
    const path = this._pathForType(type, customerId);
  ...
  _setCustomerIdOnObjects(objects, customerId) {
  ...
  _getResource(type, ...args) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._request('get', type, customerId, resourceId, params).then(data => {

        if (data.hasOwnProperty('items')) {

          let items = this._buildObjects(data.items);

          items = this._setCustomerIdOnObjects(items, customerId);
          ...
          let item = this._buildObjects(data);

          item = this._setCustomerIdOnObjects(item, customerId);
