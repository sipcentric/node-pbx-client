...
export default function(testParams) {
  ...
  describe(objectType, function() {
    ...
    before(function() {
      ...
      newObject = (client, properties, customerId) =>
        new NvObject(client, properties, customerId);
    ...
    describe("Constructor", function() {
      describe(`new ${objectType}();`, function() {
        ...
        it("accepts an object of properties and sets them on the initialized object ", function() {
          ...
          const representationObjectWithProperties = newObject(
            client,
            properties
          );
        ...
        it("cannot have it's type overridden on initialization", function() {
          ...
          assert.equal(representationObjectWrongType.type, objectType);
        });
      });
      ...
      describe("Inherited prototypes", function() {
        describe("prototype.save();", function() {
          ...
          it("doesn't return a promise if a callback is provided", function() {
            if (skipTests) this.skip();
            assert.notEqual(
              representationObject.save(function() {}) instanceof Promise
            );
          });
          ...
          it("sets the returned PUT data on the object", function(done) {
            ...
            representationObject.save(function(err, data) {
              if (err) {
                ...
                data.hasOwnProperty("testProperty") &&
                data.testProperty === testProperty
              ) {
                done();
              } else {
                done(
                  new Error("Property not set correctly on returned object")
                );
              }
            });
          });
          ...
          it("passes through request errors correctly", function(done) {
            ...
            representationObject.save().then(
              function() {
        ...
        describe("prototype.delete();", function() {
          ...
          it("doesn't return a promise if a callback is provided", function() {
            if (skipTests) this.skip();
            assert.notEqual(
              representationObject.delete(function() {}) instanceof Promise
            );
          });
          ...
          it("passes through request errors correctly", function(done) {
            ...
            representationObject.delete().then(
              function() {
                done(new Error("Request error not passed through correctly"));
              },
              function(err) {
